                        ...
                        if self.isPixelwiseModifiable:
                            ...
                            if self.width % org_width == 0 and self.height % org_height == 0:
                                ...
                                try:
                                    ...
                                    rate_y = org_height / self.height
                                    for y_new in range(self.height):
                                        for x_new in range(self.width):
                                            x_org = int(x_new * rate_x)
                                            y_org = int(y_new * rate_y)
                                            newImg.put('{#%02x%02x%02x}' % self.img.get(x_org, y_org), (x_new, y_new))
                                            # TODO 미래에 tk version 올라갔을 때 반투명 지원 가능하도록 고쳐 두기
                                            newImg.tk.call(newImg.name, 'transparency', 'set', x_new, y_new,
                                                            self.img.tk.call(org_img.name, 'transparency', 'get', x_org, y_org))
                                    ...
                                    try:
                                        ...
                                        for y_new in range(self.height):
                                            for x_new in range(self.width):
                                                x_org = int(x_new * rate_x)
                                                y_org = int(y_new * rate_y)
                                                newImg.put('{#%02x%02x%02x}' % org_img.get(x_org, y_org), (x_new, y_new))
                                                # TODO 미래에 tk version 올라갔을 때 반투명 지원 가능하도록 고쳐 두기
                                                newImg.tk.call(newImg.name, 'transparency', 'set', x_new, y_new,
                                                                org_img.tk.call(org_img.name, 'transparency', 'get', x_org, y_org))
